...
class op_base(object):
    ...
    def read_openpifpaf(
        ...
        for _, person_data in enumerate(data):
            total_keypoints = np.array(
                person_data['keypoints'], dtype=np.float32)
            total_keypoints = total_keypoints.reshape([-1, 3])

            body_pif_keypoints = total_keypoints[0:23, :]
            pif2body25_map = [
                0, -1, 6, 8, 10, 5, 7, 9, -1,
                12, 14, 16, 11, 13, 15, 2, 1, 4, 3,
                17, 18, 19, 20, 21, 22
            ]
            body_25_keypoints = body_pif_keypoints[pif2body25_map]
            body_25_keypoints[1] = (
                body_pif_keypoints[5]+body_pif_keypoints[6])/2
            body_25_keypoints[8] = (
                body_pif_keypoints[11]+body_pif_keypoints[12])/2

            body_25_keypoints = np.stack(list(body_25_keypoints.values()))
            face_keypoints = total_keypoints[23:23+68, :]
            hand_keypoints = total_keypoints[23+68:, :]

            keypoints.append(np.concatenate(
                [body_25_keypoints,
                 hand_keypoints,
                 face_keypoints]
            )
        ...
        # logger.info("{}".format(len(keypoints)), end=' ')
        return return_item_tuple(
            keypoints,
            gender_gt
        )