...
def main(args):
    ...
    for i in tqdm(range(len(testdata))):
        ...
        if tobj==None:
            continue
        name = tobj['imagename']
        images = tobj['image'].to(device)[None,...]
        ...
        target_vis={"landmarks3d":orig_visdict['landmarks3d']}
        cv2.imwrite(os.path.join(savefolder, name + '.jpg'), deca.visualize(target_vis))
        ...
        # if args.saveMat:
        #     tmp={}
        #     tmp['landmarks2d'] = opdict['landmarks2d']
        #     tmp = util.dict_tensor2npy(tmp)
        #     tmp['bbox'] = tobj['bbox']
        #     # opdict = util.dict_tensor2npy(opdict)
        #     # savemat(os.path.join(savefolder, name + '.mat'), opdict)
        #     savemat(os.path.join(savefolder, name, name + '.mat'), tmp)
        # if args.saveVis:
        #     cv2.imwrite(os.path.join(savefolder,'vis', name + '_vis.jpg'), deca.visualize(visdict))
        #     if args.render_orig:
        #         cv2.imwrite(os.path.join(savefolder,'org', name + '_vis_original_size.jpg'), deca.visualize(orig_visdict))
        # if args.saveImages:
