...
def set_rasterizer(type='pytorch3d'):
    if type == 'pytorch3d':
        ...
        #         # ,extra_cuda_cflags = ['-std=c++14', '-ccbin=gcc']) # cuda10.2 is not compatible with gcc9. Specify gcc 7
    ...
    def render_shape(self, vertices, transformed_vertices, colors=None, background=None, detail_normal_images=None, lights=None, return_grid=False, uv_detail_normals=None, h=None, w=None):
        ...
        if background is None:
            ...
            if self.rasterizer_type == 'pytorch3d':
                _,_, org_h, org_w =background.shape
                background2 = F.interpolate(background, [self.image_size, self.image_size])
                shape_images = shaded_images*alpha_images + \
                    background2.contiguous()*(1-alpha_images)
                shape_images = F.interpolate(shape_images, [org_h, org_w])
            else:
                shape_images = shaded_images*alpha_images + \
