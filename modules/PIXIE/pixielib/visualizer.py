...
class Visualizer(object):
    ...
    def __init__(self, render_size=1024, config=None, device='cuda:0', part='body', background=None, rasterizer_type='standard'):
        ...
        self.cached_data = np.load(
            self.cfg.model.flame2smplx_cached_path, allow_pickle=True, encoding='latin1').item()
        ...
        if background is not None:
            ...
            self.bg = torch.ones(
                [1, 1, self.render_size, self.render_size], device=self.device, dtype=torch.float32)
    ...
    def _setup_renderer(self):
        ...
        self.render = SRenderY(self.render_size, obj_filename=topology_path,
                               uv_size=uv_size, rasterizer_type=self.rasterizer_type).to(self.device)
        mask = imread(self.cfg.model.face_eye_mask_path).astype(
            np.float32)/255.
        mask = torch.from_numpy(mask[:, :, 0])[None, None, :, :].contiguous()
        self.flame_face_eye_mask = F.interpolate(
            mask, [self.cfg.model.uv_size, self.cfg.model.uv_size]).to(self.device)
    ...
    def displacement2normal(self, uv_z, coarse_verts, coarse_normals=None):
        ...
        uv_detail_normals = uv_detail_normals*self.smplx_face_mask + \
    ...
    def tex_flame2smplx(self, flame_texture, smplx_texture=None):
        ...
        if torch.is_tensor(flame_texture):
            ...
            smplx_texture = torch.from_numpy(smplx_texture.transpose(
                2, 0, 1)).to(device).to(dtype).unsqueeze(0)
        else:
            smplx_texture = texture_flame2smplx(
    ...
    def light_albedo(self, opdict):
        ...
        uv_shading = self.render.add_SHlight(
            uv_coarse_normals, opdict['light'])
    ...
    def render_results(self, opdict, input_images, overlay=False, light_on=False, moderator_weight=None, use_deca=False, background=None):
        ...
        if 'displacement_map' in opdict.keys():
            ...
            face_mask = F.grid_sample(
                self.smplx_face_mask, grid, align_corners=False)
            normal_images = face_detail_normal_images * \
                face_mask + normal_images*(1-face_mask)
            shape_detail_images = self.render.render_shape(
                opdict['vertices'], opdict['transformed_vertices'], detail_normal_images=normal_images, background=bg, h=h, w=w)
        ...
        if moderator_weight is not None:
            ...
            colors = torch.tensor([1.0, 1.0, 0.9])[None, None, :].repeat(
                batch_size, nv, 1).float().to(self.device)
            for part in moderator_weight.keys():
                # import ipdb; ipdb.set_trace()
                weight = moderator_weight[part].cpu().numpy()[:, 0]
                curr_color = torch.from_numpy(
                    color_map_color(weight)).float().to(self.device)
        ...
        # import ipdb; ipdb.set_trace()
        # uvmap = opdict['texture'] if 'texture' in opdict.keys(
        # ) else opdict['albedo']
    ...
    def rotate_results(self, opdict, result_type='color', light_on=False, visdict=None, savepath='vis.gif'):
        ...
        for yaw_angle in range(0, 361, 30):
            ...
            vertices = torch.matmul(
                opdict['vertices'], global_pose.transpose(1, 2))
            transformed_vertices = util.batch_orth_proj(
                vertices, opdict['cam'])
            if result_type == 'color':
                ...
                render_ops = self.render(
                    vertices, transformed_vertices, uvmap, background=self.bg, lights=lights)
